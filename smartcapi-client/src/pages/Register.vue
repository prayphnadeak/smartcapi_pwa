...
async function onRegister() {
  ...
  try {
    ...
    // 4. Simpan session di auth store
    authStore.login(user.id, user.full_name, access_token);
    
    // Simpan juga di user store (legacy/compatibility)
    userStore.setRegistrationData({ fullName: full_name.value });
    
    // 5. Arahkan ke halaman rekam suara
    console.log('Mengarahkan ke /register-voice');
    ...
    if (e.response && e.response.data && e.response.data.detail) {
      ...
      error.value = "Network Error: Tidak dapat terhubung ke server. Pastikan backend berjalan di port 8000.";
    }
  } finally {
    loading.value = false;
  }
