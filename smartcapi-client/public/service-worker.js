const CACHE_NAME = 'smartcapi-v5-debug';
const STATIC_FILES = [
  '/offline.html',
  '/manifest.json',
...
self.addEventListener('fetch', (event) => {
  ...
  // EXCLUDE WebSocket connections from Service Worker
  // WebSocket upgrade requests are GET requests but cannot be handled by fetch/cache
  if (url.pathname.includes('/ws/')) {
    return;
  }

  // Strategy 0: Navigation (HTML) - Network First + Update Cache
  // Ensures fresh index.html is loaded for responsive layout fixes
  ...
  // Strategy 3: Stale-While-Revalidate for CSS/JS assets (Vite builds)
  // This helps prevent 'white screen' if sw serves old index.html referring to old assets
  if (url.pathname.match(/\.(js|css|png|jpg|jpeg|svg|gif)$/)) {
        event.respondWith(
            caches.match(event.request).then(cachedResponse => {
                const fetchPromise = fetch(event.request).then(networkResponse => {
                    ...
                    // Network failed
                    // If we have cached response, we are good (it was returned already if we used StaleWhileRevalidate properly, 
                    // but here we return it in the catch block if distinct)
                    // unique logic below
                });

                return cachedResponse || fetchPromise; // Return cached if available, else wait for network
            }).catch(() => {
